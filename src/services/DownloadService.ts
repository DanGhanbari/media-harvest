    ...
    this.ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'progress') {
          ...
          if (callback) {
            // console.log(`🔄 DownloadService: Progress callback invoked at ${new Date().toISOString()}`);
            callback(data.progress, data);
          } else {
            // Keep error logging minimal or suppress if it happens often during clean shutdowns
    ...
    if (this.ws?.readyState !== WebSocket.OPEN) {
      ...
      await new Promise(resolve => {
        ...
        const checkConnection = () => {
          // console.log('📥 DownloadService: Checking WebSocket state:', this.ws?.readyState);
          if (this.ws?.readyState === WebSocket.OPEN) {
